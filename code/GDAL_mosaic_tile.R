...
GDAL_mosaic_tile <- function(output_file, folder_path, return_raster = TRUE, large_tif = TRUE)
{
  ...
  buildVRT <- paste0("gdalbuildvrt", " ", gsub(tools::file_ext(output_file),
                                               "vrt", output_file), " ", folder_path)
  if (large_tif == TRUE)
  {
    VRT2TIF <- paste0("gdal_translate -co compress=LZW -co BIGTIFF=YES", " ", gsub(tools::file_ext(output_file), "vrt", output_file),
                      " ", gsub(tools::file_ext(output_file), "tif", output_file))
  } else {
    VRT2TIF <- paste0("gdal_translate -co compress=LZW", " ", gsub(tools::file_ext(output_file), "vrt", output_file),
                      " ", gsub(tools::file_ext(output_file), "tif", output_file))
  }

  system(buildVRT)
  system(VRT2TIF)
  unlink(gsub(tools::file_ext(output_file),
              "vrt", output_file))
  # Return raster
  if (return_raster) {
